...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Authenticate with Supabase + sync to Xano
      const result = await login(email, password)
      
      if (result.user) {
        // Store preference if remember me
        if (rememberMe) {
  ...
  useState(() => {
    const remembered = localStorage.getItem('ff_remember_email')
    if (remembered) {
      setEmail(remembered)
      setRememberMe(true)
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-surface-primary via-indigo-950/50 to-surface-primary flex">
      {/* Left Panel - Branding */}
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center p-16">
          <div className="flex items-center gap-3 mb-8">
            <div className="w-14 h-14 bg-gradient-to-br from-primary to-primary-hover rounded-2xl flex items-center justify-center text-3xl shadow-glow-primary">
              🦍
            </div>
            <div>
              <h1 className="text-3xl font-bold text-white">FreightFlow Pro</h1>
              <p className="text-primary">by Ape Global</p>
            </div>
          </div>
        ...
        <div className="absolute bottom-20 left-20 w-96 h-96 bg-accent-purple/20 rounded-full blur-3xl" />
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <div className="bg-surface-secondary/50 backdrop-blur-xl rounded-3xl border border-slate-700/50 p-8">
            ...
            {error && (
              <div className="mb-6 p-4 bg-accent-red/10 border border-accent-red/20 rounded-xl text-accent-red text-sm text-center">
                {error}
              </div>
            )}
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input 
                    ...
                    className="w-4 h-4 rounded border-slate-600 bg-surface-primary text-primary focus:ring-primary" 
                  />
                  <span className="text-sm text-slate-400">Remember me</span>
                </label>
                <a href="#" className="text-sm text-primary hover:text-primary-hover transition">
                  Forgot password?
                </a>
              </div>
            ...
            <div className="mt-6">
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-4 bg-surface-secondary/50 text-slate-400">or continue with</span>
                </div>
              </div>

              <div className="mt-6 grid grid-cols-2 gap-4">
                <button className="flex items-center justify-center gap-2 bg-surface-primary/50 border border-slate-600 rounded-xl py-3 text-slate-300 hover:bg-surface-tertiary transition">
            ...
            <p className="mt-8 text-center text-slate-400">
              ...
              <Link to="/signup" className="text-primary hover:text-primary-hover font-medium transition">
                Sign up free
              </Link>
            </p>
          </div>
